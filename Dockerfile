...
#     E.g.: docker.io/hexpm/elixir:1.19.1-erlang-28.1.1-debian-trixie-20260610-slim
...
# A named volume is mounted here in production. Creating it in the image first
# gives the unprivileged release user ownership on a volume's first use.
RUN mkdir -p /tamagym/storage/media/img /tamagym/storage/media/gif \
  && chown -R nobody:nogroup /app /tamagym/storage

# set runner ENV
ENV MIX_ENV="prod"

# Only copy the final release from the build stage
COPY --from=builder --chown=nobody:root /app/_build/${MIX_ENV}/rel/tamagym ./
COPY --chown=nobody:root scripts/fetch-media.sh /app/bin/fetch-media
